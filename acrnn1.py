...
def batch_norm_wrapper(inputs, is_training, decay = 0.999):
    ...
    if is_training is not None:
        ...
        with tf.compat.v1.control_dependencies([train_mean, train_var]):
...
def acrnn(inputs, num_classes=4,
    ...
    layer1 = tf.nn.dropout(layer1, rate = 1-dropout_keep_prob)
    ...
    layer2 = tf.nn.dropout(layer2, rate = 1-dropout_keep_prob)
    ...
    layer3 = tf.nn.dropout(layer3, rate = 1-dropout_keep_prob)
    ...
    layer4 = tf.nn.dropout(layer4, rate = 1-dropout_keep_prob)
    ...
    layer5 = tf.nn.dropout(layer5, rate = 1-dropout_keep_prob)
    ...
    layer6 = tf.nn.dropout(layer6, rate = 1-dropout_keep_prob)
    ...
    gru_bw_cell1 = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(cell_units, forget_bias=1.0)
    
    # Now we feed `layer_3` into the LSTM BRNN cell and obtain the LSTM BRNN output.
    outputs1, output_states1 = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=gru_fw_cell1,
